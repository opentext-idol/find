...
public abstract class SavedSearch<T extends SavedSearch<T>> {
    ...
    public String toQueryText() {
        if(CollectionUtils.isEmpty(conceptClusterPhrases)) {
            ...
            final Collection<String> quotedConcepts = conceptClusterPhrases.stream()
                    .map(clusterPhrase -> wrapQuotes(clusterPhrase.getPhrase()))
                    .collect(Collectors.toCollection(LinkedList::new));

            return '(' + queryText + ") AND " + StringUtils.join(quotedConcepts, " AND ");
        }
    }
