define([
    ...
    'moment',
    'Raphael'
], function ($, _, moment, Raphael) {

    return function (optDeps, uiDeps) {
        ...
        const jobName = optDeps.jobName;
        const pageHeight = optDeps.pageHeight;
        const dayCount = optDeps.dayCount;
        const startDate = optDeps.startDate;
        ...
        const windowWidth = $parent.width();
        const hidingDelay = 500;
        const columnWidth = Math.floor(windowWidth / dayCount);
        // const dayCount = Math.max(2, Math.floor((windowWidth / columnWidth) - 0.5));
        const interval = (dayCount * 24 * 60 * 60);

        const width = dayCount * columnWidth;
        const height = pageHeight || $spectrograph.height();
        ...
        const imageHeight = height - datePadding;
        const clusterDateFormat = 'dddd'; // for momentjs

        paper.image('api/public/themetracker/image?' + $.param({
            startDate: startDate.unix(),
            interval: interval,
            jobName: jobName,
            imageWidth: width,
            imageHeight: imageHeight
        }), 0, datePadding, width, imageHeight);
        const xScale = width / sgWidth, yScale = imageHeight / sgHeight;
