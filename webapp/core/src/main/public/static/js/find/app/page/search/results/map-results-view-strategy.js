...
], function ($, Backbone, _, configuration, addLinksToSummary, MapView, i18n) {
    "use strict";
    ...
    return function (options) {
        ...
        return {
            ...
            getMarkersFromDocumentModel: function (model, markers, color) {
                ...
                _.each(locations, function (locationValues, locationName) {
                    locationValues.forEach(function (location) {
                        ...
                        const popover = popoverTemplate ? popoverTemplate({
                            title: title,
                            i18n: i18n,
                ...
                if (!_.isEmpty(markerMap)) {
                    _.each(markerMap, function (markers, markerName) {
                        ...
                        mapView.addMarkers(markers, {
                            ...
                            name: parentLayer ? null : markerName
                        });
                    });
            ...
            reloadMarkers: function () {
                if (mapView.mapRendered()) {
                    ...
                    resultSets.forEach(function (resultSet) {
                        ...
                        resultSet.clusterLayer = clusterMarkers ? mapView.addClusterLayer(resultSet.name, resultSet.layerOptions) : null;
                        resultSet.markers = {};
                    });
            ...
            collectionsFetching: function () {
                ...
                return _.chain(resultSets).pluck('collection').pluck('fetching').some().value();
            },
            ...
            getFetchOptions: function (queryModel, fieldText, length) {
                ...
                return {
                    data: {
                        ...
                        start: allowIncrement ? length + 1 : 1,
                        max_results: allowIncrement ? length + resultsStep : resultsStep,
                        indexes: queryModel.get('indexes'),
                        field_text: newFieldText,
                        min_date: queryModel.getIsoDate ? queryModel.getIsoDate('minDate') : null,
                        max_date: queryModel.getIsoDate ? queryModel.getIsoDate('maxDate') : null,
