...
class HodDocumentsController extends DocumentsController<HodQueryRequest, HodSuggestRequest, HodGetContentRequest, ResourceIdentifier, HodQueryRestrictions, HodGetContentRequestIndex, HodSearchResult, HodErrorException> {
    ...
    @Override
    protected <SR extends SearchRequest<HodQueryRestrictions>> void addParams(final SearchRequestBuilder<SR, HodQueryRestrictions, ?> request, final String summary, final String sort) {
        ((HodSearchRequestBuilder<?, ?>) request)
                .summary(Optional.ofNullable(summary).map(Summary::valueOf).orElse(null))
                .sort(Optional.ofNullable(sort).map(Sort::valueOf).orElse(null));
    }
