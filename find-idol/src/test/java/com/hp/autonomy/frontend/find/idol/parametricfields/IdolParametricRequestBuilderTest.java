/*
 * Copyright 2015 Hewlett-Packard Development Company, L.P.
 * Licensed under the MIT License (the "License"); you may not use this file except in compliance with the License.
 */

package com.hp.autonomy.frontend.find.idol.parametricfields;

import com.hp.autonomy.frontend.find.core.parametricfields.AbstractParametricRequestBuilderTest;
import com.hp.autonomy.idol.parametricvalues.IdolParametricRequest;

public class IdolParametricRequestBuilderTest extends AbstractParametricRequestBuilderTest<IdolParametricRequest, String> {
    public IdolParametricRequestBuilderTest() {
        super(new IdolParametricRequestBuilder());
    }
}
