...
public class SearchCoreITCase extends HybridIsoTestBase {
    ...
    public SearchCoreITCase(final TestConfig config) {
        super(config);
    }
    ...
    public void testSearchResultsNotEmpty() {
        final SearchPage searchPage = getApplication().searchService().search("luke");
        for (final String title : searchPage.getSearchResultTitles(SearchPage.RESULTS_PER_PAGE)) {
