...
public class SearchPageITCase extends ABCTestBase {
    ...
    private static final String HAVEN_ERROR_MESSAGE = "Haven OnDemand returned an error while executing the search action";
    private static final String SEARCH_ERROR_MESSAGE = "an error occurred executing the search action";
    private static final String CORRECT_ERROR_MESSAGE_NOT_SHOWN = "Correct error message not shown";

    private static final Logger logger = LoggerFactory.getLogger(SearchPageITCase.class);
    ...
    public void setUp() {
        ...
        try {
            ...
            logger.error("Initial thread.sleep failed");
        }
    ...
    private void search(final String searchTerm){
        ...
        searchPage.waitForSearchLoadIndicatorToDisappear();
        assertNotEquals(searchPage.getText(), contains(HAVEN_ERROR_MESSAGE));
    }
    ...
    public void testSearchParentheses() {
        final List<String> testSearchTerms = Arrays.asList("(",")","()",") (",")war");

        if(getConfig().getType().equals(ApplicationType.HOSTED)){
            for(final String searchTerm : testSearchTerms){
                search(searchTerm);

                assertThat(searchPage.getText(),containsString(HAVEN_ERROR_MESSAGE));
            }
        } else if (getConfig().getType().equals(ApplicationType.ON_PREM)) {
            int searchTerm = 0;
            final String bracketMismatch = "Bracket Mismatch in the query";
            search(testSearchTerms.get(searchTerm++));

            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(SEARCH_ERROR_MESSAGE));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(bracketMismatch));
            ...
            search(testSearchTerms.get(searchTerm++));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(SEARCH_ERROR_MESSAGE));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString("No valid query text supplied"));

            search(testSearchTerms.get(searchTerm++));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(SEARCH_ERROR_MESSAGE));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString("Terminating boolean operator"));
    ...
    public void testSearchQuotationMarks() {
        final List<String> testSearchTerms = Arrays.asList("\"","\"\"","\" \"","\" \"","\"word","\" word","\" wo\"rd\"");
        ...
        if(getConfig().getType().equals(ApplicationType.HOSTED)){
            ...
            final String noValidQueryText = "No valid query text supplied";
            final String unclosedPhrase = "Unclosed phrase";
            int searchTerm = 0;

            search(testSearchTerms.get(searchTerm++));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(SEARCH_ERROR_MESSAGE));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(unclosedPhrase));

            search(testSearchTerms.get(searchTerm++));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(SEARCH_ERROR_MESSAGE));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(noValidQueryText));
            ...
            search(testSearchTerms.get(searchTerm++));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString("An error occurred executing the search action"));
            assertThat(CORRECT_ERROR_MESSAGE_NOT_SHOWN, searchPage.getText(), containsString(unclosedPhrase));
    ...
    public void testDeleteDocsFromWithinBucket() {
        ...
        for (final String bucketDocTitle : bucketList) {
            ...
            assertFalse("The document title appears as blank within the bucket for document titled " + searchPage.getSearchResult(bucketList.indexOf(bucketDocTitle) + 1).getText(), bucketDocTitle.isEmpty());
...
//      searchPage.selectAllIndexesOrDatabases(getConfig().getType().getName());
        ...
        searchPage.waitForSearchLoadIndicatorToDisappear();
        assertThat("Field Text should not have caused an error", searchPage.getText(), not(containsString(HAVEN_ERROR_MESSAGE))); //TODO this should really be assertThat but it really doesn't like it for some reason
        searchPage.waitForSearchLoadIndicatorToDisappear();
...
//        indexesWarn();
        ...
        searchPage.loadOrFadeWait();
        searchPage.waitForSearchLoadIndicatorToDisappear();
        assertThat("Field Text should not have caused an error", searchPage.getText(), not(containsString(HAVEN_ERROR_MESSAGE)));
...
//        }
        ...
        assertThat("Sum of 'A NOT B', 'B NOT A' and 'A AND B' should equal 'A OR B' where A is: red  and B is: star",
            fourthSearchCount + redNotStar + starNotRed, is(secondSearchCount));    //TODO Check
    }
...
//      searchPage.selectLanguage("English", getConfig().getType().getName());
    ...
    public void testInvalidQueryTextNoKeywordsLinksDisplayed() {
        ...
        final List<String> boolOperators = Arrays.asList("OR", "WHEN", "SENTENCE", "DNEAR");
        final List<String> stopWords = Arrays.asList("a", "the", "of", "SOUNDEX"); //According to IDOL team SOUNDEX isn't considered a boolean operator without brackets

        if(getConfig().getType().equals(ApplicationType.HOSTED)) {
            languageWarn();

            final List<String> allTerms = ListUtils.union(boolOperators, stopWords);
...
//      searchPage.selectAllIndexesOrDatabases(getConfig().getType().getName());
        ...
        assertThat("Document should not be visible. Date filter not working", firstResult, not(searchPage.getSearchResultTitle(1)));

        searchPage.openFromDatePicker();
        datePicker = new DatePicker(searchPage.$el(), getDriver());
        datePicker.calendarDateSelect(DateUtils.addMinutes(date, -1));
        searchPage.closeFromDatePicker();
        assertThat("Document should be visible. Date filter not working", firstResult, not(searchPage.getSearchResultTitle(1)));
    }
...
//      assertNotEquals("From date cannot be after the until date", searchPage.fromDateTextBox().getAttribute("value"), "04/05/2000 12:01 PM");
        assertEquals("From date should be blank", searchPage.fromDateTextBox().getAttribute("value"), "");
...
//      assertEquals("Until date cannot be before the from date", searchPage.untilDateTextBox().getAttribute("value"),is(not("04/05/2000 11:59 AM")));
        assertEquals("Until date should be blank", searchPage.untilDateTextBox().getAttribute("value"),"");
    }
...
//      searchPage.closeUntilDatePicker();
    ...
    public void testRelatedConceptsDifferentInDifferentLanguages() {
        ...
        assertThat("Concepts should be different in different languages", englishConcepts, not(frenchConcepts));
...
//        }
        ...
        assertThat("Page should not have thrown an error", searchPage.getText(), not(containsString(HAVEN_ERROR_MESSAGE)));
    ...
    public void testNoRelatedConceptsIfNoResultsFound() {
        ...
        final String errorMessage = "Garbage text returned results. garbageQueryText string needs changed to be more garbage like";
